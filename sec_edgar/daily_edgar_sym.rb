...
def parse_edgar(cmp_detail = {})
  begin
    ...
    #system 'sudo rm -rf /home/manan/download/$todays_date/'
    ...
    quote = YahooFinance::get_quotes(YahooFinance::ExtendedQuote, cmp_ticker).values.first
    stquote = YahooFinance::get_quotes(YahooFinance::StandardQuote, cmp_ticker).values.first
    if !stquote.nil?
      shrp = stquote.lastTrade
    else
      shrp = "N/A"
    end
    if !quote.nil?
        ...
        if rev_each.context.entity.segment.nil?
          ...
          revs_append.each do |rev_append|
            if ((rev_each.def["id"] == rev_append.to_s) && (rev_each.def["type"] == "xbrli:monetaryItemType"))
              ...
              cpy_detail[cmp_cik.to_sym][:revenue][edate.to_s][period.to_sym] = rev_each.value.to_f
            end
          end
        else
    ...
    if item_eps.nil?
      ...
      if !item_eps.empty?
        item_eps.each do |eps_each|
          ...
          cpy_detail[cmp_cik.to_sym][:eps][edate.to_s][period.to_sym] = eps_each.value.to_f
          end
      end
    end
    #pp $cmp_assets
...
def print_edgar(cmp_detail = {},parse_detail = {})
  ...
  unless cmp_revs.nil?
    ...
    if cmp_revs[periods[1]][:MTHS_3].to_s !~ /\{/
      this_rev = cmp_revs[periods[1]][:MTHS_3]
      last_rev = cmp_revs[periods[0]][:MTHS_3]
      if last_rev.blank?
        puts "In some cases, the period[1] doesnot have entry for 3_MTHS REV....See if period_all[0] has it, for #{cmp_cik}"
        last_revs = "N/A"
        if cmp_revs[periods_all[1]][:MTHS_6].to_s !~ /\{/
          last_rev = cmp_revs[periods_all[1]][:MTHS_6]
        elsif cmp_revs[periods_all[0]][:MTHS_6].to_s !~ /\{/
          last_rev = cmp_revs[periods_all[0]][:MTHS_6]
        else
    ...
    elsif cmp_revs[periods[1]][:MTHS_12].to_s !~ /\{/
      this_rev = cmp_revs[periods[1]][:MTHS_12]
      last_rev = cmp_revs[periods[0]][:MTHS_12]

      periods_all = cmp_revs.keys.sort
      if last_rev.blank?
        puts "In some cases, the period[1] doesnot have entry for 12_MTHS REV....See if period_all[0] has it"
        if cmp_revs[periods_all[0]][:MTHS_12].to_s !~ /\{/
          last_rev = cmp_revs[periods_all[0]][:MTHS_12]
        else
          if cmp_revs[periods_all[1]][:MTHS_6].to_s !~ /\{/
            last_rev = cmp_revs[periods_all[1]][:MTHS_6]
          elsif cmp_revs[periods_all[0]][:MTHS_6].to_s !~ /\{/
  ...
  unless cmp_eps.nil?
    ...
    if cmp_eps[periods[1]][:MTHS_3].to_s !~ /\{/
      this_eps = cmp_eps[periods[1]][:MTHS_3]
      last_eps = cmp_eps[periods[0]][:MTHS_3]
      if last_eps.blank?
        puts "In some cases, the period[1] doesnot have entry for 3_MTHS EPS....See if period_all[0] has it"
        if cmp_eps[periods_all[0]][:MTHS_3].to_s !~ /\{/
          last_eps = cmp_eps[periods_all[0]][:MTHS_3]
        else
          if cmp_eps[periods_all[1]][:MTHS_6].to_s !~ /\{/
            last_eps = cmp_eps[periods_all[1]][:MTHS_6]
          elsif cmp_eps[periods_all[0]][:MTHS_6].to_s !~ /\{/
            last_eps = cmp_eps[periods_all[0]][:MTHS_6]
          else
    ...
    elsif cmp_eps[periods[1]][:MTHS_12].to_s !~ /\{/
      this_eps = cmp_eps[periods[1]][:MTHS_12]
      last_eps = cmp_eps[periods[0]][:MTHS_12]

      periods_all = cmp_eps.keys.sort
      if last_eps.blank?
        puts "In some cases, the period[1] doesnot have entry for 12_MTHS EPS....See if period_all[0] has it"
        if cmp_eps[periods_all[0]][:MTHS_12].to_s !~ /\{/
          last_eps = cmp_eps[periods_all[0]][:MTHS_12]
        else
          if cmp_eps[periods_all[1]][:MTHS_6].to_s !~ /\{/
            last_eps = cmp_eps[periods_all[1]][:MTHS_6]
          elsif cmp_eps[periods_all[0]][:MTHS_6].to_s !~ /\{/
  ...
  if ( (mcap_val !~ /^N\/A/) && (cur_stock_eq.to_s !~ /\{/) )
    #puts "mcap  #{mcap_val}"
    #puts "stock_holder_eq #{cur_stock_eq}"
    sthe_mcap_ratio = (cur_stock_eq.to_f / mcap_val.to_f).round(2)
    #puts "ste/mcap ratio is #{sthe_mcap_ratio}"
  else
    if (mcap_val =~ /^N\/A/)
      #puts "Can't calulate sthe/mcap ratio as MCAP is N/A"
    elsif ( (cur_stock_eq.to_s =~ /\{/) && (cur_liab.to_s !~ /\{/))
      cur_stock_eq = cur_ass.to_f - cur_liab.to_f
      #puts "Stock_holder_eq is N/A , but cur_liab is available, calculate stock_holder_eq = #{cur_stock_eq} ....ass -liab"
      sthe_mcap_ratio = (cur_stock_eq.to_f / mcap_val.to_f).round(2)
    else
      #puts "Can't calculate sthe/mcap ratio, as sthe & liab both are not available"
      sthe_mcap_ratio = "N/A"
    end
  end

  #pp cmp_ass_info
  string = cmp_name.to_s + "," + cmp_ticker.to_s + "," + cmp_cik.to_s + "," +  fil_date.to_s + "," + this_period.to_s + "," + cur_ass.to_s  + "," + pct_chg_ass.to_s + "," + cur_liab.to_s  + "," + pct_chg_liab.to_s + "," + this_eps.to_s  +  "," + eps_chg.to_s   + "," + anly_est.to_s  + "," + anly_chg.to_s +  "," + anly_price.to_s +  "," + ass_liab_ratio.to_s  +  "," + sthe_mcap_ratio.to_s + "," + shrp.to_s + "," + div.to_s +  "," + mcap_mil.to_s + "M," + cur_stock_out.to_s + "," + pct_stk_chg.to_s + "," + cur_rev.to_s + "," + pct_rev_chg.to_s + "\n"
...
#pp cmp_details
string = "cmp_name,cmp_ticker,cmp_cik,File_Date,This_period,Curr_Ass,Chg_Ass%,Curr_Liab,Chg_liab%,C_EPS,Chg_EPS%,ANL_EST,ANL_EPS%,ANL_PT,ASS/LIAB,STHE/MCAP,SHRP,DIV,MCAP,STK_OUT,%Chg_STK,REV,%CHG_REV"
...
cmp_details.each do |cmp_detail_str|
  cmp_detail = symbolize_keys(cmp_detail_str)
  if(rerun == 1)
