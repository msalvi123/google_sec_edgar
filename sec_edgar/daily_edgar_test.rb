...
def parse_edgar(cmp_detail = {})
  ...
  cpy_detail[cmp_cik.to_sym]["analyst_est"] = FinModeling::Marketwatch.parse_ticker(cmp_ticker)
...
def print_edgar(cmp_detail = {},parse_detail = {})
  ...
  unless cmp_stk_outs.nil?
    ...
    if ( (this_value.to_s =~ /\{/) || (this_value.to_s =~ /\{/) )
      ...
      cmp_stkout_info["%CHG"] = (100*( this_value.to_i - last_value.to_i)/ last_value.to_i.abs).to_f.round(2)
    end
  end
  ...
  unless cmp_eps.nil?
    ...
    if cmp_eps[periods[1]][":3_MTHS"].to_s !~ /\{/
      ...
      if !anly_est_hoa.empty?
        qtr_anly_est = anly_est_hoa[:"Mean Estimate"][0]
      else
        qtr_anly_est = "N/A"
      end
      ...
      if !anly_est_hoa.empty?
        yly_anly_est = anly_est_hoa[:"Mean Estimate"][2]
      else
        yly_anly_est = "N/A"
      end
      cmp_eps_info[":TV"] = this_eps
...
#pp cmp_details
string = "cmp_name,cmp_ticker,cmp_cik,File_Date,This_period,Curr_Ass,Chg_Ass%,Curr_Liab,Chg_liab%,C_EPS,Chg_EPS%,ANL_EST,ANL_EPS%,ASS/LIAB,STHE/MCAP,SHRP,DIV,MCAP,STK_OUT,%Chg_STK,REV,%CHG_REV"

rerun = 0
last_line = ""
if File.exists?($all_data_csv)
  File.readlines($all_data_csv, 'r').each do |line|
    last_line = line
  end
  rerun = 1
  #pp last_line
...
cmp_details.each do |cmp_detail|
  #next unless cmp_detail["cmp_cik"] == "0001555538"
  if rerun == 1
    next unless last_line =~ /#{cmp_detail["cmp_cik"]}/
    rerun = 0
  end
...
$file_all_data.close
system("rm -rf #{DOWNLOAD_PATH_DATE}")
